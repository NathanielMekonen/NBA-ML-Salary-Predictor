...
best_contracts_df <- read_csv(
  "~/Desktop/Data_Projects/nba_ml_salary_analysis/data/model_output/best_value_contracts.csv",
  col_select = -1
)
...
best_contracts_table <- best_contracts_df %>%
  mutate(
    Player_Image = paste0(
      logo,
      "|||",
  ...
  tab_header(
    title = html(
      paste0(
        ...
        "<span>The NBA's Best Value Contracts</span>",
    ...
    subtitle = "Players whose 2025-26 production exceeds their current salary"
  ) %>%
...
best_contracts_table

gtsave(
  best_contracts_table,
  "images/best_contract_values.png"
)
